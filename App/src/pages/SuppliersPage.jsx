...
export default function SuppliersPage({ refresh }) {
  ...
  const purchaseCard = (purchase, ordered) => <Paper key={purchase.id} variant="outlined" className="purchase-plan-card"><Stack direction="row" gap={1} sx={{ justifyContent: 'space-between', alignItems: 'flex-start' }}><Box><Typography fontWeight={900}>{purchase.purchaseNumber}</Typography><Typography variant="body2" color="text.secondary">{purchase.supplier?.name || 'ကုန်သည်'} · {String(purchase.orderedAt || '').slice(0, 10)}</Typography>{purchase.expectedAt ? <Typography variant="body2" color="text.secondary">ပို့မည့်ရက် — {String(purchase.expectedAt).slice(0, 10)}</Typography> : null}</Box><Chip size="small" label={ordered ? 'ပို့လာရန်' : 'မှတ်ထားသည်'} color={ordered ? 'warning' : 'default'} /></Stack><Divider sx={{ my: 1 }} />{(purchase.items || []).map((item) => <Typography key={item.id} variant="body2">{item.productName} · {item.quantity} ခု · မှာစျေး {money(item.plannedUnitCost ?? item.unitCost)}{item.plannedPromotionLabel ? ` · ${item.plannedPromotionLabel}` : ''}</Typography>)}<Stack direction="row" sx={{ mt: 1, justifyContent: 'flex-end' }}>{ordered ? <Button size="small" variant="contained" onClick={() => setReceiving({ ...purchase, items: purchase.items.map((item) => ({ ...item, actualUnitCost: item.unitCost })), confirmed: false, needsConfirmation: false })}>လက်ခံမည်</Button> : <Button size="small" variant="contained" onClick={() => send(purchase)} disabled={busy}>မှာပြီး</Button>}</Stack></Paper>
