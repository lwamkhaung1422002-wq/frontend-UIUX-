    ...
    <Box sx={{ minHeight: "100vh", display: "grid", placeItems: "center", px: 2, py: 3, bgcolor: "background.default" }}>
      <Container maxWidth="xs" disableGutters>
        <Stack spacing={2.5} sx={{ alignItems: "center", mb: 3 }}>
          ...
          <Box sx={{ textAlign: "center" }}><Typography variant="h4" fontWeight={800}>General POS</Typography><Typography color="text.secondary" sx={{ mt: .5 }}>{isRegister ? "Create your store account" : "Sign in to manage your store"}</Typography></Box>
        </Stack>
        <Card elevation={0} sx={{ borderRadius: 2.5, border: "1px solid", borderColor: "divider" }}>
          <CardContent sx={{ p: { xs: 2.5, sm: 3.5 }, "&:last-child": { pb: { xs: 2.5, sm: 3.5 } } }}>
            <Stack component="form" onSubmit={submit} spacing={2}>
              ...
              <TextField required type="password" autoComplete={isRegister ? "new-password" : "current-password"} slotProps={{ htmlInput: { minLength: isRegister ? 8 : undefined } }} label="Password" value={form.password} onChange={update("password")} helperText={isRegister ? "At least 8 characters" : undefined} sx={fieldSx} />
              ...
              <Typography variant="body2" color="text.secondary" sx={{ textAlign: "center" }}>{isRegister ? "Already have an account?" : "New to General POS?"} <Link component={RouterLink} to={isRegister ? "/login" : "/register"} underline="hover" fontWeight={700}>{isRegister ? "Sign in" : "Create an account"}</Link></Typography>
            </Stack>
          </CardContent>
        </Card>
      </Container>
