...
function CostHistoryDialog({ open, onClose, history, loading, error }) {
  ...
  return <Dialog open={open} onClose={onClose} fullWidth maxWidth="md"><DialogTitle sx={{ fontWeight: 700 }}>Cost Price History</DialogTitle><DialogContent dividers>{loading && <Box sx={{ py: 4, display: "grid", placeItems: "center" }}><CircularProgress size={28} /></Box>}{error && <Typography color="error">{error}</Typography>}{!loading && !error && history.length === 0 && <Typography color="text.secondary">No stock-in or adjustment cost records yet.</Typography>}{!loading && !error && history.length > 0 && <><Box sx={{ ...gridSx, pb: 1, borderBottom: "1px solid", borderColor: "divider" }}><Typography color="text.secondary" sx={{ fontSize: 12, fontWeight: 700 }}>DATE</Typography><Typography color="text.secondary" sx={{ fontSize: 12, fontWeight: 700 }}>QUANTITY</Typography><Typography color="text.secondary" sx={{ fontSize: 12, fontWeight: 700 }}>UNIT COST</Typography><Typography color="text.secondary" sx={{ fontSize: 12, fontWeight: 700 }}>AVG COST</Typography><Typography color="text.secondary" sx={{ fontSize: 12, fontWeight: 700 }}>SOURCE</Typography></Box>{history.map((entry) => <Box key={entry.id} sx={{ ...gridSx, py: 1.5, borderBottom: "1px solid", borderColor: "divider", "&:last-child": { borderBottom: 0 } }}><Typography sx={{ fontWeight: 700, fontSize: 14 }}>{formatDate(entry.date)}</Typography><Typography sx={{ fontSize: 14, color: entry.direction === "OUT" ? "error.main" : "success.main" }}>{entry.direction === "OUT" ? "-" : "+"}{Number(entry.stockIn)} pcs</Typography><Typography sx={{ fontSize: 14 }}>{formatMoney(entry.unitCost)}</Typography><Typography sx={{ fontSize: 14, fontWeight: 700 }}>{entry.averageCost === null ? "—" : formatMoney(entry.averageCost)}</Typography><Typography color="text.secondary" sx={{ fontSize: 14 }}>{entry.source}</Typography></Box>)}</>}</DialogContent></Dialog>;
}
