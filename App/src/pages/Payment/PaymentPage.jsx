...
async function loadPaymentRecordsLegacy(api) {
  const [purchasesResult, expensesResult, ordersResult] = await Promise.all([
    api.purchases.list({ pageSize: 100 }),
    api.expenses.list(),
    api.orders.list({ pageSize: 100 }),
  ]);
  const suppliers = (purchasesResult.purchases || []).map((purchase) => {
    const paid =
      Number(purchase.paidAmount || 0) >= Number(purchase.total || 0);
    const payment = [...(purchase.payments || [])].sort(
      (a, b) => new Date(b.paidAt) - new Date(a.paidAt),
    )[0];
    const date = toIsoDate(
      paid
        ? payment?.paidAt || purchase.updatedAt
        : purchase.expectedAt || purchase.createdAt,
    );
    return {
      id: purchase.purchaseNumber || purchase.id,
      apiId: purchase.id,
      supplierId: purchase.supplierId,
      name: purchase.supplier?.name || "Supplier",
      amount: paid
        ? Number(purchase.paidAmount || purchase.total || 0)
        : Math.max(
            0,
      ...
      dateLabel: paid ? "Paid" : "Due",
      date,
      isoDate: date,
      kind: "supplier",
      sortAt: new Date(purchase.createdAt).getTime(),
    };
  });
  const expenses = (expensesResult.expenses || []).map((expense) => {
    const date = toIsoDate(expense.spentAt || expense.createdAt);
    const kind = expense.category === "income" ? "income" : "expense";
    return {
      id: expense.id,
      apiId: expense.id,
      name: expense.title,
      ...
      status: "Paid",
      method: toMethod(expense.method),
      dateLabel: "Paid",
      date,
      isoDate: date,
      kind,
      remark: expense.note || "",
      sortAt: new Date(expense.createdAt || expense.spentAt).getTime(),
    };
  });
  const sales = (ordersResult.orders || []).map((order) => {
    ...
    return {
      ...
      remainingAmount: Math.max(0, Number(order.total || 0) - paidAmount),
      hasPaymentRecord: (order.payments || []).length > 0,
      status,
      method: toMethod(latestPayment?.method),
      dateLabel:
  ...
  return [...suppliers, ...expenses, ...sales].sort(
    (left, right) =>
      (right.sortAt || new Date(`${right.isoDate}T00:00:00`).getTime()) -
      (left.sortAt || new Date(`${left.isoDate}T00:00:00`).getTime()),
  );
}
