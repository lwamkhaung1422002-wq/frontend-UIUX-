...
export default function AddPromotionPage() {
  ...
  const save = async () => { if (!form.name.trim() || !form.start || !form.end || !form.reason.trim()) { setError("Name, dates, and reason are required."); return; } if (scope === "individual" && !selected) { setError("Select a product."); return; } const type = form.manualPrice ? "FIXED_PRICE" : "PERCENTAGE"; const value = Number(form.manualPrice || form.percentage); if (!Number.isFinite(value) || value <= 0) { setError("Enter a valid discount."); return; } setSaving(true); setError(""); try { await api.pricing.createPromotionCampaign({ name: form.name, scope: scope === "individual" ? "PRODUCT" : scope.toUpperCase(), ...(scope === "individual" ? { productId: selected.id } : {}), ...(scope === "category" ? { categoryId: category } : {}), type, value, startsAt: new Date(form.start).toISOString(), endsAt: new Date(`${form.end}T23:59:59`).toISOString(), state: "SCHEDULED", reason: form.reason }); await Promise.all([queryClient.invalidateQueries({ queryKey: queryKeys.promotionCampaigns(shop?.id) }), queryClient.invalidateQueries({ queryKey: queryKeys.pricing(shop?.id) }), queryClient.invalidateQueries({ queryKey: queryKeys.products(shop?.id) }), queryClient.invalidateQueries({ queryKey: queryKeys.dashboard(shop?.id) }), queryClient.invalidateQueries({ queryKey: ["shops", shop?.id, "reports"] })]); navigate("/price"); } catch (err) { setError(err.message || "Promotion could not be saved."); } finally { setSaving(false); } };
