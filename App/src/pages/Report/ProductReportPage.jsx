...
export default function ProductReportPage() {
  ...
  const reportQuery = useMemo(() => {
    ...
    return { search, ...range, ...(category !== "All categories" ? { categoryId: category } : {}) };
  }, [category, dateRange, from, search, to]);
  ...
  return <Box sx={{ maxWidth: 1500, mx: "auto", py: 1 }}>
    ...
    <Box sx={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 2, mb: 2.5 }}>
      <SummaryCard icon={<Inventory2RoundedIcon />} tone="blue" label="Total Products" value={String(reportData.summary.totalProducts)} helper="Active Products" />
      <SummaryCard icon={<Inventory2RoundedIcon />} tone="green" label="Total Quantity" value={String(reportData.summary.totalQuantity)} helper="In Stock" />
      <SummaryCard icon={<WarningAmberRoundedIcon />} tone="orange" label="Low Stock Items" value={String(reportData.summary.lowStockCount)} helper="Need Attention" />
      <SummaryCard icon={<ErrorOutlineRoundedIcon />} tone="red" label="Out of Stock Items" value={String(reportData.summary.outOfStockCount)} helper="Currently Out" />
    </Box>
    ...
    <Popover open={Boolean(filterAnchor)} anchorEl={filterAnchor} onClose={() => setFilterAnchor(null)} anchorOrigin={{ vertical: "bottom", horizontal: "right" }} transformOrigin={{ vertical: "top", horizontal: "right" }} slotProps={{ paper: { sx: { width: 260, p: 2, borderRadius: 2 } } }}>
      ...
      <TextField select fullWidth size="small" label="Category" value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map((item) => <MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}</TextField>
