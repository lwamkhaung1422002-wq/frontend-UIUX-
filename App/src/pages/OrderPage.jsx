  ...
  return <Box className="page-stack pos-page">
    ...
    <Box className="pos-layout"><Paper className="pos-register" elevation={0}>
      ...
      <Stack divider={<Divider flexItem />}>{cart.map((item, index) => <Box className="pos-line" key={item.productId}>
        <Box className="pos-line-description"><Typography fontWeight={800}>{item.code} · {item.name}</Typography>{item.promotionDiscount > 0 ? <><Typography variant="caption" display="block" sx={{ textDecoration: 'line-through' }}>တစ်ခုလျှင် {money(item.regularPrice)}</Typography><Box className="pos-line-discount"><Typography component="span" variant="caption">{item.promotionName || 'လျှော့စျေး'}</Typography><Typography component="strong" variant="caption">လျှော့စျေး − {money(item.promotionDiscount * item.quantity)}</Typography></Box><Typography variant="caption" color="text.secondary" display="block">{item.promotionCondition}</Typography></> : <Typography variant="caption">တစ်ခုလျှင် {money(item.unitPrice)}</Typography>}<Typography className="pos-stock-available" variant="caption" display="block">လက်ကျန် {Number(item.availableQuantity || 0).toLocaleString('en-US')} မျိုး</Typography></Box>
        ...
        <Box className="pos-line-price"><Typography className="pos-line-total">{money(item.quantity * item.unitPrice)}</Typography></Box>
      </Box>)}</Stack>
