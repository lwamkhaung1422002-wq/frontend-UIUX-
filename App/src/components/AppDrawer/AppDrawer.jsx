...
export default function AppDrawer({ expanded, setExpanded }) {
  ...
  const [reportsOpen, setReportsOpen] = useState(pathname.startsWith("/report"));
  ...
  const reportSubItem = (label, path) => <ListItemButton key={path} selected={pathname === path} onClick={() => navigate(path)} sx={{ minHeight: 42, mb: .5, pl: 5.75, pr: 1, borderRadius: 1.5, color: "rgba(255,255,255,.92)", "&.Mui-selected": { bgcolor: "rgba(255,255,255,.14)", color: "#fff" }, "&:hover": { bgcolor: "rgba(255,255,255,.10)" } }}><ListItemText primary={label} slotProps={{ primary: { sx: { fontSize: 13, fontWeight: 600, lineHeight: 1.25 } } }} /></ListItemButton>;
  const reportMenu = <><Tooltip title={expanded ? "" : "Reports & Analytics"} placement="right"><ListItemButton selected={pathname.startsWith("/report")} onClick={() => { if (!expanded) navigate("/report"); setReportsOpen((value) => !value); }} sx={navItemSx}><ListItemIcon sx={iconSx}><ReportIcon /></ListItemIcon><ListItemText primary="Reports & Analytics" sx={textSx} slotProps={{ primary: { sx: { fontSize: 14, fontWeight: 600 } } }} />{expanded && (reportsOpen ? <ExpandLessIcon fontSize="small" /> : <ExpandMoreIcon fontSize="small" />)}</ListItemButton></Tooltip><Collapse in={expanded && reportsOpen} timeout="auto" unmountOnExit>{reportSubItem("Sale Reports & Analytics", "/report/sales")}{reportSubItem("Product Reports & Analytics", "/report/products")}</Collapse></>;
  ...
  return <Drawer variant="permanent" onMouseEnter={() => setExpanded(true)} onMouseLeave={() => setExpanded(false)} sx={{ width: drawerWidth, flexShrink: 0, transition: "width 180ms ease", "& .MuiDrawer-paper": { width: drawerWidth, boxSizing: "border-box", overflowX: "hidden", background: "linear-gradient(180deg, #106fd5 0%, #0764ca 100%)", color: "#fff", borderRight: 0, transition: "width 180ms ease" } }}>
    ...
    <List sx={{ px: expanded ? 1.5 : 1.25, py: 1.75, flexGrow: 1 }}>{menuItems.slice(0, -1).map(listItem)}{reportMenu}{listItem(menuItems.at(-1))}</List>
